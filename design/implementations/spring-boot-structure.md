...
Status: 실제 구현 파일 기준 · 순수 예약 검증 분리 · 2026-09-16
...
| validation/ReservationValidation | 제품 존재 boolean과 요청·replay product 일치의 순수 업무 검증 |
| repositories/ReservationRepository | entity persist·claim flush·typed replay 변환·재고 변경/제품 존재 사실·예약 저장 조합 |
...
`ReservationReplayEntity`는 저장된 `product_id`와 `Instant` 정밀도를 그대로 `Reservation`으로 변환하고,
Service는 repository가 반환한 replay·재고 변경·제품 존재 사실을 순수 검증에 전달합니다.
...
예약 validation 분기는 기존 HTTP 제품 미존재·normal replay와 JPA post-claim replay 회귀에서 확인합니다.
